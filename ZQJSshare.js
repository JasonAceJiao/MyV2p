...
Quantumuil X：添加远程重写
[rewrite_remote]
https://gitee.com/curtinlv/qx/raw/master/rewrite/youth.conf, tag=中青安卓极速版 by Curtin, update-interval=172800, opt-parser=false, enabled=true

中青安卓极速版分享一篇文章到自己的微信上，自己点击一下即触发会自动完成10好有阅读奖励 500青豆/次。

 */
const $ = new Env("中青安卓极速版分享阅读-助力10次");
  ...
  try {
    if ($request.headers && $request.url.indexOf("user.youth.com/count2") > -1) {
      ...
      for(let i=1;i<11;i++){
        ...
        $.notify("中青安卓极速版分享", "", "️据获取失败");
      }
    } catch (eor) {
    $.msg("中青安卓极速版数据获取失败", "", "️中青安卓极速版数据获取失败");
  }
